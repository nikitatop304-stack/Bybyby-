...
async def check_access_and_execute(func, *args, **kwargs):
    """Проверяет подписку и выполняет функцию"""
...
async def choose_gift_handler(callback: types.CallbackQuery):
    # Проверка доступа
    result = await check_access_and_execute(_choose_gift_handler, callback)
    if result is False:
        return

async def _choose_gift_handler(callback: types.CallbackQuery):
...
async def process_gift(callback: types.CallbackQuery):
    result = await check_access_and_execute(_process_gift, callback)
    if result is False:
        return

async def _process_gift(callback: types.CallbackQuery):
    user_id = callback.from_user.id
...
async def process_sticker(callback: types.CallbackQuery):
    result = await check_access_and_execute(_process_sticker, callback)
    if result is False:
        return

async def _process_sticker(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    game = user_games.get(user_id)
...
async def my_stats_handler(callback: types.CallbackQuery):
    result = await check_access_and_execute(_my_stats_handler, callback)
    if result is False:
        return

async def _my_stats_handler(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    user = get_user(user_id)
...
async def get_daily_bonus(callback: types.CallbackQuery):
    result = await check_access_and_execute(_get_daily_bonus, callback)
    if result is False:
        return

async def _get_daily_bonus(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    user = get_user(user_id)
    ...
    await _my_stats_handler(callback)

# ========== ПРИГЛАШЕНИЕ ДРУЗЕЙ ==========
@dp.callback_query(F.data == "invite")
async def invite_friend_handler(callback: types.CallbackQuery):
    result = await check_access_and_execute(_invite_friend_handler, callback)
    if result is False:
        return

async def _invite_friend_handler(callback: types.CallbackQuery):
    user_id = callback.from_user.id
...
async def buy_attempts_handler(callback: types.CallbackQuery):
    result = await check_access_and_execute(_buy_attempts_handler, callback)
    if result is False:
        return

async def _buy_attempts_handler(callback: types.CallbackQuery):
    kb = InlineKeyboardBuilder()
...
async def process_purchase(callback: types.CallbackQuery):
    result = await check_access_and_execute(_process_purchase, callback)
    if result is False:
        return

async def _process_purchase(callback: types.CallbackQuery):
    user_id = callback.from_user.id
...
async def help_handler(callback: types.CallbackQuery):
    result = await check_access_and_execute(_help_handler, callback)
    if result is False:
        return

async def _help_handler(callback: types.CallbackQuery):
